...
public interface IReportHeartRateRepository extends JpaRepository<HeartRate, Long> {
    ...
    @Query("SELECT heartRate FROM HeartRate heartRate WHERE heartRate.user.id = ?2 and date(heartRate.measureTime) = ?1 order by heartRate.heartRate desc LIMIT 1")
    HeartRate getMaxHeartRateOnDay(Date date, Long userId);
    @Query("SELECT heartRate FROM HeartRate heartRate WHERE heartRate.user.id = ?1 and heartRate.measureTime between ?2 And ?3")
    List<HeartRate> getHeartRateOnLastWeek(Long userId, Date startDate, Date endDate);

    @Query("SELECT heartRate FROM HeartRate heartRate WHERE heartRate.user.id = ?1 AND date(heartRate.measureTime) = ?2")
    List<HeartRate> getHeartRateOnDay(Long userId, Date convertedDate);

    @Query("select heartRate FROM HeartRate heartRate WHERE heartRate.user.id = ?1 and date(heartRate.measureTime) BETWEEN ?2 and ?3")
    ...
    @Query("select  heartRate From HeartRate heartRate where heartRate.user.id = ?4 " +
            "and date(heartRate.measureTime) = ?1 " +
            "and HOUR(heartRate.measureTime) BETWEEN ?2 AND  ?3")
    ...
    @Query("select  heartRate From HeartRate heartRate where heartRate.user.id = ?1 " +
            "and date(heartRate.measureTime) BETWEEN ?2 AND  ?3" +
            "and HOUR(heartRate.measureTime) BETWEEN ?4 AND  ?5")
